...
class EquivariantResEncoder76Cyclic(torch.nn.Module):
    def __init__(self, obs_channel: int = 2, n_out: int = 128, initialize: bool = True, N=8):
        ...
        self.conv = torch.nn.Sequential(
            # 76x76
            nn.R2Conv(
                nn.FieldType(self.group, obs_channel * [self.group.trivial_repr]),# 输入：标量场（无旋转响应）
                nn.FieldType(self.group, n_out // 8 * [self.group.regular_repr]),# 输出：正则表示场（每个通道对应群的一个旋转操作）
                kernel_size=5,
                padding=0,
                initialize=initialize,
            ),
