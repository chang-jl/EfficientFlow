...
from EfficientFlow.model.equi.equi_encoder import EquivariantResEncoder76Cyclic, EquivariantVoxelEncoder58Cyclic, EquivariantVoxelEncoder64Cyclic
from EfficientFlow.model.vision.voxel_crop_randomizer import VoxelCropRandomizer
...
class EquivariantObsEnc(ModuleAttrMixin):
    def __init__(
        ...
        self.token_type = nn.FieldType(self.group, self.n_hidden * [self.group.regular_repr])# 256*8
    ...
    def forward(self, nobs):
        obs = nobs["agentview_image"] #[128, 2, 3, 84, 84]
        ...
        enc_out = self.enc_obs(obs).tensor.reshape(batch_size * t, -1)  # 2*b 2048
        ih_out = self.enc_ih(ih) #2*b 256
        pos_xy = ee_pos[:, 0:2]
        pos_z = ee_pos[:, 2:3]
        features = torch.cat(
            [
        ...
        features = nn.GeometricTensor(features, self.enc_out.in_type)#2*b 2315
        out = self.enc_out(features).tensor#2*b 2048
