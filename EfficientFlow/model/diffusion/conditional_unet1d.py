...
from EfficientFlow.model.diffusion.conv1d_components import (
    Downsample1d, Upsample1d, Conv1dBlock)
from EfficientFlow.model.diffusion.positional_embedding import SinusoidalPosEmb
...
class ConditionalUnet1DShortcut(nn.Module):
    def __init__(self, 
        input_dim,
        local_cond_dim=None,
        global_cond_dim=None,
    ...
    def forward(self, 
        ...
        if not torch.is_tensor(dt):
            dt = torch.tensor([dt], dtype=torch.long, device=sample.device)
        elif torch.is_tensor(dt) and len(dt.shape) == 0:
            dt = dt[None].to(sample.device)
        dt = dt.expand(sample.shape[0])

        global_feature = self.t_encoder(t) + self.dt_encoder(dt)
...
class MeanflowConditionalUnet1D(nn.Module):
    def __init__(self, 
        input_dim,
        local_cond_dim=None,
        global_cond_dim=None,
    ...
    def forward(self, 
        ...
        """
        x: (B,T,input_dim)
        timestep t: (B,) or int, diffusion step
        local_cond: (B,T,local_cond_dim)
        global_cond: (B,global_cond_dim)
        output: (B,T,input_dim)
        """
        ...
        global_feature = self.t_encoder(timesteps)+ self.r_encoder(timesteps_r)
