...
class ExpandLayer(nn.Module):
    """
    可学习维度扩展层，将 hidden_size 从 64 映射到 1024
    输入形状: [*, seq_len, hidden_size]
    输出形状: [*, seq_len, out_features]
    """
    def __init__(self, hidden_size=64, out_features=1024):
        super().__init__()
        # 1. 归一化层（保持与 FinalLayer 一致的设计）
        ...
        self.expand = Mlp(
            ...
            hidden_features=hidden_size,  # 保持隐藏层维度与输入一致
            out_features=out_features,    # 目标输出维度 1024
            act_layer=approx_gelu,        # 使用近似 GELU
            drop=0                        # 无 dropout
        )
