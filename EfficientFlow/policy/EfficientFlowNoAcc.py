...
class EfficientFlowNoAccPolicy(BasePolicy):
    def __init__(self, 
            shape_meta: dict, 
            horizon, 
            n_action_steps, 
        ...
        self.use_preaction_as_noise=use_preaction_as_noise
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor],preaction=None) -> Dict[str, torch.Tensor]:
        ...
        if preaction is None:
            self.Predict_actionTimesFlag = 0
    ...
    def compute_loss(self, batch):
        ...
        vt = self.diff(xt, t*99, local_cond=local_cond, global_cond=global_cond)
        vt[condition_mask] = cond_data[condition_mask]   
        vt = torch.nan_to_num(vt)
    
        loss = torch.nn.functional.mse_loss(vt,(target-a0))
